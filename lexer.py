...
def is_alphanum(c: str) -> bool:
    # Maiúsculas e minusculas sem acento + digitos
    return c.isascii() and c.isalnum()

#Funcao que efetua a análise propriamente dita
def analyze(source: str) -> None:
    ...
    #Adiciona uma quebra de linha ao final do código-fonte para possibilitar o processamento do último lexema
    source += "\n"
    ...
    def display_error(ch: str, teminal: int) -> tuple[str,int]:
        pass

    pos = 0 #Posição atual de leitura dentro do código-fonte
    while pos < len(source):
        ch = source[pos]   #Lê um caracter

        #Se for uma quebra de linha, ajuste a contagem de linhas e colunas
        if ch == "\n":
            row += 1
            col = 0
        ...
        match state:
            ...
            case 40:
                if ch == 'd': lexeme, state = go_to_state(ch, 50)
                elif ch in BLANKS: lexeme, state = accept(ch, 1002)
                else: display_error(ch)
